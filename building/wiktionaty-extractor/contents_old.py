...
class Contents :
    def __init__(self) :
	self.separator1_re = re.compile(ur'={2,4}\s?\{\{[\w\s]+\|[\w\s]+\}\}\s?\w*?\s?={2,4}(.*?)(?===)',re.UNICODE | re.DOTALL)
	self.separator2_re = re.compile(ur'={2,4}\s?[Ff]orma\s?[\w\s]+\s?={2,4}(.*?)(?===)',re.UNICODE | re.DOTALL)
	self.title_re = re.compile(r'<title>([^\W\d_]+)</title>',re.UNICODE)
	self.word_specs_re = re.compile(ur'={2,4}\s?\{\{([\w\s]+)\|([\w\s]+)\}\}\s?(\w*?)?\s?={2,4}',re.UNICODE)  #FIND A MORE CIVILIZED WAY TO DO THIS
	...
	new_specs = []
	for i in specs :
	    new_specs_item = [it for it in i]
	    try :
		new_specs_item[1] = lang.LANGUAGES[i[1]]
	    except KeyError :
		new_specs_item[1] = 'lengua desconocida'
	    new_specs += [new_specs_item]
	return new_specs
